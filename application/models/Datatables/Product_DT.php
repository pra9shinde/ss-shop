<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	private function _get_datatables_query()
    {
        ...
        $this->db->select('product.id, product.name as product_name, category.name as category_name, product.total_quantity, product.rem_quantity, product.price,product.pieces', false);
    ...
    public function load_multi_btns($obj,$row_data){
        $btns = '<div class="btn-group mx-2" role="group" aria-label="Second Group">';
        $btns .= '<button type="button" data-toggle="modal" class="btn btn-icon btn-warning" onclick=\'editSocFloor(';
        $btns .= json_encode($obj);
        $btns .= ',';
        $btns .= json_encode($row_data);
        $btns .= ')\'><i class="la la-edit"></i></button>';
        $btns .= '<button type="button" class="btn btn-icon btn-danger" onclick="deleteSocFloor(';
        $btns .= $obj->id;
